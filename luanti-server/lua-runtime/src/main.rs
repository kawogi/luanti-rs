//! test implementation for loading the Luanti's builtin Lua scripts
...
fn main() -> Result<()> {
    ...
    let builtin_path = PathBuf::from("../../luanti-org/luanti/builtin").canonicalize()?;
    info!("{}", builtin_path.display());
    let main_path = builtin_path.join("init.lua");
    let builtin_init_lua = read_to_string(main_path)?;

    let lua = Lua::new();
    let globals = lua.globals();

    globals.set("core", create_core_table(&builtin_path, &lua)?)?;
    globals.set("debug", create_debug_table(&lua)?)?;
    globals.set("DIR_DELIM", MAIN_SEPARATOR_STR)?;
    globals.set("INIT", "game")?;

    lua.load(builtin_init_lua)
        .set_name("@builtin/init.lua")
        .exec()?;

    Ok(())
}

fn create_core_table(builtin_path: &Path, lua: &Lua) -> Result<mlua::Table> {
    let core = lua.create_table()?;

    let core_get_us_time = lua.create_function(|_, ()| {
        SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_micros())
            .map_err(|error| mlua::Error::RuntimeError(error.to_string()))
    })?;
    core.set("get_us_time", core_get_us_time)?;

    let core_log = lua.create_function(|_, (level, message): (String, String)| {
        match level.as_str() {
            "none" | "info" => info!("{message}"),
            "warning" => warn!("{message}"),
            "error" => error!("{message}"),
            "action" => info!("ACTION: {message}"),
            "deprecated" => warn!("DEPRECATED: {message}"),
    ...
    core.set("log", core_log)?;

    let Some(builtin_path_str) = builtin_path.to_str().map(|path| {
        // ensure that the return path contains a trailing path separator
        ...
        return Err(mlua::Error::RuntimeError(format!(
            "builtin path contains invalid characters: '{}'",
            builtin_path.display()
        )));
    };
    let core_get_builtin_path = lua.create_function(move |_, ()| Ok(builtin_path_str.clone()))?;
    core.set("get_builtin_path", core_get_builtin_path)?;

    Ok(core)
}

fn create_debug_table(lua: &Lua) -> Result<mlua::Table> {
    let debug = lua.create_table()?;
    let debug_getinfo = lua.create_function(|lua, (value, options): (Value, Option<String>)| {
        let options = options.as_ref().map_or("flnSu", String::as_str);
        ...
        let stack_level = if value.is_integer() {
            let stack_level = value.as_usize();
            if stack_level.is_none() {
                return Err(mlua::Error::RuntimeError(format!(
                    "invalid stack frame selected: {value:?}"
                )));
            }
            stack_level
            ...
            return Err(mlua::Error::RuntimeError(
                "function or level expected".into(),
            ));
        };

        let Some(debug) = lua.inspect_stack(stack_level.unwrap_or(0)) else {
            return Err(mlua::Error::RuntimeError(format!(
                "invalid stack frame selected: {value:?}"
            )));
        };

        let function_info = value.as_function().map(Function::info);

        let debug_info = lua.create_table()?;

        if let Some(function_info) = function_info {
            if options.contains('S') {
                debug_info.set("source", function_info.source)?;
                debug_info.set("short_src", function_info.short_src)?;
                debug_info.set("linedefined", function_info.line_defined)?;
                debug_info.set("lastlinedefined", function_info.last_line_defined)?;
                debug_info.set("what", function_info.what)?;
            }
            if options.contains('n') {
                debug_info.set("name", function_info.name)?;
                debug_info.set("name_what", function_info.name_what)?;
            }
        } else {
            if options.contains('S') {
                let source = debug.source();
                debug_info.set("source", source.source)?;
                debug_info.set("short_src", source.short_src)?;
            }
            if options.contains('n') {
                let names = debug.names();
                debug_info.set("name", names.name)?;
                debug_info.set("name_what", names.name_what)?;
            }
        }

        if options.contains('l') {
            debug_info.set("currentline", debug.curr_line())?;
        }

        if options.contains('u') {
            debug_info.set("nups", debug.stack().num_ups)?;
        }
        ...
        Ok(Value::Table(debug_info))
    })?;
    debug.set("getinfo", debug_getinfo)?;
    Ok(debug)
}
...
fn _main() -> Result<()> {
    ...
    // You can create userdata with methods and meta-methods defined on them.
